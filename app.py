...
def generate_docx():
    ...
    if mindmap:
        ...
        # 將 Mermaid 語法轉換為圖片（需要實現）
        try:
            # 這裡需要實現將 Mermaid 轉換為圖片的邏輯
            mindmap_image_path = convert_mermaid_to_image(mindmap)
            if mindmap_image_path:
                doc.add_picture(mindmap_image_path, width=Inches(6.0))
                os.remove(mindmap_image_path)  # 刪除臨時圖片
        except Exception as e:
            ...
            if rows:
                ...
                for row in rows:
                    ...
                    for i, cell in enumerate(cells):
                        new_row[i].text = cell.text.strip()
                        # 如果是表頭，設置粗體
                        if row == rows[0]:
    ...
    file_path = os.path.join(download_dir, download_name)
    
    # 儲存文件
    doc.save(file_path)
    
    # 返回下載連結
    download_url = url_for('static', filename=f"downloads/{download_name}", _external=True)
